...
def run_ema_crossover(
    *,
    ...
    files_dir, plots_dir = make_single_ticker_output_dirs(
        ...
        run_name=run_name,
        run_root=run_root,
        strategy_folder="ema_crossover",
    )

    df["fast_ema"] = df["close"].ewm(span=fast_window, adjust=False, min_periods=fast_window).mean()
    df["slow_ema"] = df["close"].ewm(span=slow_window, adjust=False, min_periods=slow_window).mean()
    ...
    config = {
        ...
        "signal_rule": "invested if fast EMA > slow EMA; otherwise cash; signal shifted by 1 day",
    }

    account_path = files_dir / f"{ticker.lower()}_ema_crossover_account_values.csv"
    metrics_path = files_dir / f"{ticker.lower()}_ema_crossover_metrics.csv"
    config_path = files_dir / f"{ticker.lower()}_ema_crossover_config.json"
