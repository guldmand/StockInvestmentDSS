"""
Momentum point-in-time baseline for a single ticker.

Signal logic:
    - Compute the price return over a lookback window (pct_change).
    - If momentum is positive (lookback return > 0), target position is 100% invested.
    - Otherwise, target position is 100% cash.
    - Position is shifted by one trading day to avoid same-day look-ahead.

Point-in-time safety:
    pct_change(periods=lookback_window) on day t uses the close price on day t
    and day t-lookback_window.  The subsequent shift(1) shifts the resulting
    signal forward by one day so that no day-t momentum value affects the
    day-t position.

Output contract:
    Writes four files to:
        outputs/run_registry/algorithmic_baselines/<dataset_tag>/momentum/<ticker>/

    - <ticker>_momentum_account_values.csv   — time series with momentum and signals
    - <ticker>_momentum_metrics.csv          — performance metrics (Etape 2 canonical)
    - <ticker>_momentum_config.json          — run parameters
    - <ticker>_momentum_account_value.png    — portfolio value plot
...
def run_momentum(
    *,
    ...
    """
    Run a momentum strategy for one ticker and write outputs to the V2 canonical
    run directory.
    ...
    lookback_window:
        Number of trading days over which the price return is computed.
    ...
    account_path = data_dir / f"{ticker.lower()}_momentum_account_values.csv"
    metrics_path = metrics_dir / f"{ticker.lower()}_momentum_metrics.csv"
    config_path = config_dir / f"{ticker.lower()}_momentum_config.json"
