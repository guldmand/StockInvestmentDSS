...
def main() -> int:
    ...
    try:
        ...
        # Hold diagnostic stats derived from existing training_df columns
        _masked_to_hold_count = (
            int(training_df["action_was_masked"].sum())
            if not training_df.empty and "action_was_masked" in training_df.columns
            else None
        )
        _total_train_steps = len(training_df) if not training_df.empty else 0
        _masked_action_rate = (
            float(_masked_to_hold_count / _total_train_steps)
            if _masked_to_hold_count is not None and _total_train_steps > 0
            else None
        )
        _total_req = sum(training_action_counts.values()) if training_action_counts else 0
        _hold_pct_requested = (
            float(training_action_counts.get("HOLD", 0) / _total_req)
            if _total_req > 0
            else None
        )
        _total_eff = sum(effective_action_counts.values()) if effective_action_counts else 0
        _hold_pct_effective = (
            float(effective_action_counts.get("HOLD", 0) / _total_eff)
            if _total_eff > 0
            else None
        )
        ...
        summary = {
            ...
            "iqn": {
                ...
                "masked_to_hold_count": _masked_to_hold_count,
                "masked_action_rate": _masked_action_rate,
                "hold_pct_requested": _hold_pct_requested,
                "hold_pct_effective": _hold_pct_effective,
                "final_eval": final_eval,
            },
            "point_in_time_split": {
        ...
        # Write standalone hold diagnostic summary for post-run analysis
        hold_diag = {
            "run_id": run_paths.run_id,
            "seed": random_seed,
            "total_training_steps": _total_train_steps,
            "requested_action_counts": training_action_counts,
            "effective_action_counts": effective_action_counts,
            "masked_to_hold_count": _masked_to_hold_count,
            "masked_action_rate": _masked_action_rate,
            "hold_pct_requested": _hold_pct_requested,
        ...
        hold_diag_path = run_paths.summary_directory / "hold_diagnostic_summary.json"
        write_json(hold_diag_path, hold_diag)
