...
def get_metadata_value(metadata: dict[str, Any], *keys: str, default: Any = None) -> Any:
...
def build_data_provenance_summary(
    *,
    ...
    metadata = getattr(daily_data_result, "metadata", {}) if daily_data_result is not None else {}
    if not isinstance(metadata, dict):
        metadata = {}

    result_tickers = list(getattr(daily_data_result, "tickers", []) or [])
    requested_tickers = list(tickers or result_tickers or metadata.get("tickers", []) or [])

    failure_message = str((failure or {}).get("message") or "")
    missing_expected_tickers = parse_missing_expected_tickers_from_error(failure_message)

    metadata_failed_tickers = get_metadata_value(
        metadata,
        "failed_tickers",
        "failed_downloads",
    ...
    metadata_downloaded_tickers = get_metadata_value(
        metadata,
        "downloaded_tickers",
        "successful_tickers",
    ...
    successful_tickers = sorted(str(item).upper() for item in metadata_downloaded_tickers)
    ...
    return {
        ...
        "final_source_used": (
            ...
            if download_success
            else "partial_or_failed_download"
            if download_attempted
            else "unknown"
        ),
        ...
        "download_method_attempted": get_metadata_value(metadata, "download_method_attempted"),
...
def score_action_distribution(
    ...
    if score_mode == "q50_minus_cvar_penalty":
        return q50 - risk_lambda * abs(cvar10)

    if score_mode == "mean_plus_cvar10":
        return mean + cvar10

    raise ValueError(
        f"Unsupported IQN score mode: {score_mode}. "
        "Use mean, q50_minus_cvar_penalty, or mean_plus_cvar10."
    )
        ...
        decision_rows.append(
            {
                ...
                "selected_ticker_holdings_delta": (
                    executed_holdings_delta.get(selected_ticker) if selected_ticker else None
                ),
                ...
                "holdings_delta_json": json.dumps(executed_holdings_delta, sort_keys=True),
            }
        )
...
def main() -> int:
    ...
    try:
        ...
        explicit_tickers_text = (
            get_environment_variable("STOCK_INVESTMENT_DSS_FINRL_TICKERS", default=None)
            or get_environment_variable("STOCK_INVESTMENT_DSS_DAILY_DATA_TICKERS", default=None)
        )
        universe_id = (
                ...
                wandb_run.config.update(
                    {
                        ...
                        "point_in_time_split.train_row_count": int(len(split_result.train_data)),
                        "point_in_time_split.trade_row_count": int(len(split_result.trade_data)),
                    },
        ...
        initial_eval_summary, initial_distribution_rows, _initial_asset_memory, initial_step_rows = (
            evaluate_iqn_agent(
                agent=agent,
                tickers=tickers,
        ...
        for step in range(1, total_steps + 1):
            ...
            if step == 1 or step % log_interval == 0 or step == total_steps:
                wandb_log_metrics(
                    wandb_run,
                    {
                        ...
                        "train/portfolio_value_after": state_after.get("portfolio_value"),
            ...
            if step % eval_interval == 0 or step == total_steps:
                eval_summary, distribution_rows, _asset_memory, step_rows = evaluate_iqn_agent(
                    agent=agent,
                    tickers=tickers,
                    trade_data=split_result.trade_data,
                    step=step,
                ...
                wandb_run.summary.update(
                    {
                        ...
                        "loss_max": max(losses) if losses else None,
                        "loss_mean": float(np.mean(losses)) if losses else None,
                        "final_eval/final_value": final_eval_for_wandb.get("final_value"),
                        "final_eval/total_return_pct": final_eval_for_wandb.get("total_return_pct"),
                        "final_eval/annualized_sharpe": final_eval_for_wandb.get("annualized_sharpe"),
                        "final_eval/max_drawdown_pct": final_eval_for_wandb.get("max_drawdown_pct"),
                        "final_eval/cvar_pct": final_eval_for_wandb.get("cvar_pct"),
                        "final_eval/action_counts": final_eval_for_wandb.get("action_counts"),
                        ...
                        "experiment/market_data_start": experiment_context.get("market_data_start"),
                        "experiment/market_data_end": experiment_context.get("market_data_end"),
                        "experiment/pit_cutoff": experiment_context.get("pit_cutoff"),
                        "result/profit_loss": experiment_context.get("profit_loss"),
                        "result/total_return_pct": experiment_context.get("total_return_pct"),
                        "result/made_money": experiment_context.get("made_money"),
                        "data/final_source_used": data_provenance.get("final_source_used"),
                        "data/download_attempted": data_provenance.get("download_attempted"),
                        "data/download_success": data_provenance.get("download_success"),
                        "data/cache_used": data_provenance.get("cache_used"),
                        "data/import_file_used": data_provenance.get("import_file_used"),
                        "data/failed_ticker_count": len(data_provenance.get("failed_tickers") or []),
                    }
                )
            except Exception:
            ...
            try:
                ...
                if wandb_run is not None:
                    try:
                        ...
                        for path in [
                            run_paths.summary_directory / "experiment_context_summary.md",
                            run_paths.summary_directory / "experiment_context_summary.json",
                            run_paths.summary_directory / "experiment_failure_summary.md",
                            run_paths.summary_directory / "experiment_failure_summary.json",
                        ...
                        run_logger.exception("Failed to log W&B failure provenance artifact.")
            except Exception:
                pass
